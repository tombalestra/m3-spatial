...
from xy_utils.distributed import is_main_process
from xy_utils.visual import visualize_combined, visualize_combined_all
...
def training(dataset_args, opt_args, pipe_args, args, log_file):
    ...
    gaussians = GaussianModel(dataset_args.sh_degree, dataset_args.emb_degree, args.use_embed) # initialize the GaussianModel
    
    with torch.no_grad():
        scene = Scene(args, gaussians) # Load train/eval data, and initialize the point cloud, and the GaussianModel
        gaussians.training_setup(opt_args) # Initialize the optimizer and the learning rate scheduler
    ...
    for iteration in range(
        ...
        if args.local_sampling:
            ...
            batched_all_cameras_idx = batched_all_cameras_idx.view(-1).cpu().numpy().squeeze()
            batched_cameras = train_dataset.get_batched_cameras_from_idx(
                batched_all_cameras_idx
            )
        else:
            # Default not local sampling
        ...
        if args.backend == "gsplat":
            ...
            batched_screenspace_pkg = distributed_preprocess3dgs_and_all2all_final(
                ...
                batched_strategies=batched_strategies,
                mode="train",
            ) # preprocess parameter for rendering
            batched_image, batched_embeddings, batched_compute_locally = render_final(
                batched_screenspace_pkg, batched_strategies, use_embed=args.use_embed
            ) # render the image
            batch_statistic_collector = [
                cuda_args["stats_collector"]
                for cuda_args in batched_screenspace_pkg["batched_cuda_args"]
        ...
        loss_sum, batched_losses, dict_losses = batched_loss_computation(
            batched_image,
            batched_embeddings,
            batched_cameras,
            ...
            scene.emb_mem_ops,
            args,
        ) # compute the loss according to the rendered image
        
        # scale loss_sum by the batch size
        loss_sum = loss_sum / (args.bsz // utils.WORLD_SIZE * 1.0)
        ...
        with torch.no_grad():
            # log wandb
            if is_main_process() and iteration // args.bsz % 30 == 0 and args.use_wandb:
                wandb.log(dict_losses, step=iteration)
            ...
            _batched_losses = []
            for data in batched_losses:
                if data['Ll1'] > 0 and data['ssim_loss'] > 0:
                    _batched_losses.append([data['Ll1'], data['ssim_loss']])
            batched_losses = torch.tensor(_batched_losses, device="cuda")
            ...
            # Densification: If the cuurent gaussian states needs to be densified
            if args.backend == "gsplat":
            ...
            if config["cameras"] and len(config["cameras"]) > 0:
                ...
                for idx in range(1, num_cameras + 1, args.bsz):
                    ...
                    if args.local_sampling:
                        ...
                        batched_all_cameras_idx = (
                            batched_all_cameras_idx.view(-1).cpu().numpy().squeeze()
                        )
                    ...
                    for camera_id, (image, embedding, gt_camera) in enumerate(
                        zip(batched_image, batched_embeddings, batched_cameras)
                    ):
                        if (
                        ...
                        # Save image, gt_image, and feature
                        if embedding is not None:
                            visual_folder = args.visual_path
                            os.makedirs(visual_folder, exist_ok=True)
                            image_name = gt_camera.image_name
                            visualize_combined_all(args, gt_image, image, embedding, scene.emb_proj_ops, scene.emb_mem_ops, visual_folder + f"/{image_name}.png")
                ...
                if is_main_process() and args.use_wandb:
                    dict_results = {
                        "l1_{}".format(config["name"]): l1_test,
                        "psnr_{}".format(config["name"]): psnr_test,
                    }
                    wandb.log(dict_results, step=iteration)
