...
from xy_utils.io import resolution_minmax
...
def readJsonInfo(info):
    if info is None:
        return None
    
    info_dict = {}
    for dp in info['images']:
        info_dict[dp['file_name']] = dp
    return info_dict
...
def readColmapCameras(cam_extrinsics, cam_intrinsics, images_folder):
    ...
    # TODO: Temp Solution
    embed_folder = {
        "clip": images_folder.replace("images", "clip"),
        "dinov2": images_folder.replace("images", "dinov2"),
        "llama3": images_folder.replace("images", "llama3"),
        "seem": images_folder.replace("images", "seem"),
        "siglip": images_folder.replace("images", "siglip"),
        "llamav": images_folder.replace("images", "llamav"),
    }

    # This part is extremely slow, we need to optimize it
    embed_info = {
        "clip": readJsonInfo(json.load(open(images_folder.replace("images", "clip_info.json"))) if args.use_clip else None),
        "dinov2": readJsonInfo(json.load(open(images_folder.replace("images", "dinov2_info.json"))) if args.use_dinov2 else None),
        "llama3": readJsonInfo(json.load(open(images_folder.replace("images", "llama3_info.json"))) if args.use_llama3 else None),
        "seem": readJsonInfo(json.load(open(images_folder.replace("images", "seem_info.json"))) if args.use_seem else None),
        "siglip": readJsonInfo(json.load(open(images_folder.replace("images", "siglip_info.json"))) if args.use_siglip else None),
        "llamav": readJsonInfo(json.load(open(images_folder.replace("images", "llamav_info.json"))) if args.use_llamav else None),
    }
    ...
    for idx, key in tqdm(
        ...
        width, height = resolution_minmax(image.size, args.max_resolution)
        
        this_embed_info = {
            model_type: {image_name: values[image_name]} 
            for model_type, values in embed_info.items() 
            if values is not None and image_name in values
        }
...
def readTraceSceneInfo(path, images, eval, llffhold=10):
    try:
        cameras_extrinsic_file = os.path.join(path, "sparse/0", "images.bin")
        cameras_intrinsic_file = os.path.join(path, "sparse/0", "cameras.bin")
        cam_extrinsics = read_extrinsics_binary(cameras_extrinsic_file)
