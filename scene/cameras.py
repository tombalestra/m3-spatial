...
class Camera(nn.Module):
    def __init__(
        ...
        if (
            ...
            and (not args.web_render) if hasattr(args, 'web_render') else True
        ):
            # load to cpu
            self.original_image_backup = image.contiguous()

            # Temp Solution
            # Data loading currently especially for embedding size is not generalized to all models.
            self.original_embeddings_backup = {}
            if embeddings is not None:
                for model_type, values in embeddings.items():
                    self.original_embeddings_backup[model_type] = values["embeddings"].contiguous()

            if args.preload_dataset_to_gpu:
                self.original_image_backup = self.original_image_backup.to("cuda")
                if embeddings is not None:
                    for model_type, values in self.original_embeddings_backup.items():
                        self.original_embeddings_backup[model_type] = values.to("cuda")

            self.embeddings_size = {}
            if embeddings is not None:
                for model_type, values in embeddings.items():
                    self.embeddings_size[model_type] = {"height": values['emb_height'], "width": values['emb_width']}
    ...
    def __repr__(self):
        return f"Camera(colmap_id={self.colmap_id}, image_name={self.image_name}, uid={self.uid}), FoVx={self.FoVx}, FoVy={self.FoVy}, R={self.R}, T={self.T}, trans={self.trans}, scale={self.scale})"
