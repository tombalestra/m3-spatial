...
class GaussianModel:
    ...
    def __init__(self, sh_degree: int, emb_degree: int, use_embed: bool):
        self.active_sh_degree = 0
        self.max_sh_degree = sh_degree
        self.use_embed = use_embed # NOTE XYZ: Learnable embeddings
        self.emb_degree = emb_degree # NOTE XYZ: Learnable embeddings
        self._xyz = torch.empty(0)
        self._features_dc = torch.empty(0)
        self._features_rest = torch.empty(0)
        self._embeddings = torch.empty(0) # NOTE XYZ: Learnable Features
    ...
    def capture(self):
        return (
            ...
            self._features_dc,
            self._features_rest,
            self._embeddings, # NOTE XYZ: Learnable Features
            self._scaling,
            self._rotation,
            self._opacity,
            self.max_radii2D,
    ...
    def create_from_pcd(self, pcd: BasicPointCloud, spatial_lr_scale: float):
        ...
        # for each RGB channel, we have a SH representation
        features[:, :3, 0] = fused_color
        features[:, 3:, 1:] = 0.0
        # So this feature is actually represent color in SH domain for each gaussians.
        # embeddings = torch.zeros((fused_color.shape[0], self.emb_degree, 1)).float().cuda() # NOTE XYZ: Learnable embeddings
        # TODO: We can edit the initialization method.
        embeddings = nn.Embedding(fused_color.shape[0], self.emb_degree).weight[:,:,None].float().cuda() # NOTE XYZ: Learnable embeddings
        ...
        if (
            ...
            embeddings = embeddings[point_ind_l:point_ind_r].contiguous() # NOTE XYZ: Learnable Features
        ...
        self._xyz = nn.Parameter(fused_point_cloud.requires_grad_(True)) # Spatial position of the gaussians
        self._features_dc = nn.Parameter(
            features[:, :, 0:1].transpose(1, 2).contiguous().requires_grad_(True)
        ) # Color initialization of the gaussians, according to the original color
        self._features_rest = nn.Parameter(
            features[:, :, 1:].transpose(1, 2).contiguous().requires_grad_(True)
        ) # Additional color initialization of the gaussians
        self._embeddings = nn.Parameter(
            embeddings.transpose(1, 2).contiguous().requires_grad_(True)
        ) # NOTE XYZ: Learnable Features
        self._scaling = nn.Parameter(scales.requires_grad_(True))
        self._rotation = nn.Parameter(rots.requires_grad_(True))
        self._opacity = nn.Parameter(opacities.requires_grad_(True))
        self.max_radii2D = torch.zeros((self.get_xyz.shape[0]), device="cuda") # Max radius of the gaussians in 2D
    ...
    def training_setup(self, training_args):
        ...
        l = [
            ...
            {
                "params": [self._embeddings], # NOTE XYZ: Learnable Features
                "lr": training_args.feature_lr / 5.0, # TODO XYZ: Adjust lr for mmm feature via adding new hyperparameter
                "name": "f_emb",
            },
            {
    ...
    def construct_list_of_attributes(self):
        ...
        # NOTE XYZ: Learnable Features
        for i in range(self._embeddings.shape[1] * self._embeddings.shape[2]):
            l.append("f_emb_{}".format(i))
    ...
    def save_ply(
        ...
        if args.gaussians_distribution and not args.distributed_save:
            ...
            _embeddings = gather_uneven_tensors(self._embeddings) # NOTE XYZ: Learnable Features
            ...
            _embeddings = self._embeddings # NOTE XYZ: Learnable Features
        ...
        attributes = np.concatenate(
            (xyz, normals, f_dc, f_rest, f_emb, opacities, scale, rotation), axis=1
        ) # NOTE XYZ: Learnable Features
    ...
    def distributed_load_ply(self, folder):
        ...
        catted_embeddings = [] # NOTE XYZ: Learnable Features
        catted_opacity = []
        catted_scaling = []
        catted_rotation = []
        for rk in range(world_size):
            ...
            xyz, features_dc, features_extra, embeddings, opacities, scales, rots = (
            ...
            catted_embeddings.append(embeddings) # NOTE XYZ: Learnable Features
        ...
        catted_embeddings = np.concatenate(catted_embeddings, axis=0) # NOTE XYZ: Learnable Features
        ...
        # NOTE XYZ: Learnable Features
        self._embeddings = nn.Parameter(
            torch.tensor(catted_embeddings, dtype=torch.float, device="cuda")
            .transpose(1, 2)
            .contiguous()
            .requires_grad_(True)
        )
    ...
    def load_raw_ply(self, path):
        ...
        emb_f_names = [
            p.name
            for p in plydata.elements[0].properties
            if p.name.startswith("f_emb_")
        ]
        embeddings = np.zeros((xyz.shape[0], len(emb_f_names)))
        for idx, attr_name in enumerate(emb_f_names):
            embeddings[:, idx] = np.asarray(plydata.elements[0][attr_name])
        embeddings = embeddings.reshape(
            (embeddings.shape[0], self.emb_degree, 1)
        )
        ...
        if args.gaussians_distribution and utils.WORLD_SIZE > 1:
            ...
            embeddings = np.ascontiguousarray(
                embeddings[point_ind_l:point_ind_r]
            )
        ...
        if args.drop_initial_3dgs_p > 0.0:
            ...
            embeddings = embeddings[drop_mask] # NOTE XYZ: Learnable Features
            scales = scales[drop_mask]
            rots = rots[drop_mask]
            opacities = opacities[drop_mask]

        # NOTE XYZ: Learnable Features
        return xyz, features_dc, features_extra, embeddings, opacities, scales, rots

    def one_file_load_ply(self, folder):
        path = os.path.join(folder, "point_cloud.ply")
        xyz, features_dc, features_extra, embeddings, opacities, scales, rots = self.load_raw_ply(
            path
        ) # NOTE XYZ: Learnable Features
        ...
        # NOTE XYZ: Learnable Features
        self._embeddings = nn.Parameter(
            torch.tensor(embeddings, dtype=torch.float, device="cuda")
            .transpose(1, 2)
            .contiguous()
            .requires_grad_(True)
        )
    ...
    def prune_points(self, mask):
        ...
        self._embeddings = optimizable_tensors["f_emb"] # NOTE XYZ: Learnable Features
        self._opacity = optimizable_tensors["opacity"]
    ...
    def densification_postfix(
        ...
        new_features_dc,
        new_features_rest,
        new_embeddings, # NOTE XYZ: Learnable Features
        new_opacities,
        new_scaling,
        new_rotation,
        ...
        d = {
            ...
            "f_emb": new_embeddings, # NOTE XYZ: Learnable Features
    ...
    def densify_and_split(self, grads, grad_threshold, scene_extent, N=2):
        ...
        new_embeddings = self._embeddings[selected_pts_mask].repeat(N, 1, 1) # NOTE XYZ: Learnable Features
        new_opacity = self._opacity[selected_pts_mask].repeat(N, 1)
        new_send_to_gpui_cnt = self.send_to_gpui_cnt[selected_pts_mask].repeat(N, 1)
        
        # TODO XYZ: Split Learnable Features
    ...
    def densify_and_clone(self, grads, grad_threshold, scene_extent):
        ...
        new_embeddings = self._embeddings[selected_pts_mask] # NOTE XYZ: Learnable Features
...
def sync_gradients_sparsely(gaussians, group):
    with torch.no_grad():
        ...
        sync_grads(gaussians._embeddings) # NOTE XYZ: Learnable Features
    ...
    with torch.no_grad():
        ...
        all_params_grads = [
            ...
            for param in [
                ...
                gaussians._embeddings, # NOTE XYZ: Learnable Features
                gaussians._opacity,
                gaussians._scaling,
                gaussians._rotation,
            ]
