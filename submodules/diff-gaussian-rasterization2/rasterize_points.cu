...
RenderGaussiansCUDA(
	...
	torch::Tensor& emb,//NOTE XYZ: Learnable embeddings
	...
	const pybind11::dict &args)
{
  ...
  torch::Tensor out_embeddings = torch::full({EMB_CHANNELS, H, W}, 0.0, float_opts); //NOTE XYZ: Learnable embeddings
  ...
  if(P != 0)
  {
	  rendered = CudaRasterizer::Rasterizer::renderForward(
		...
		emb.contiguous().data<float>(),//NOTE XYZ: Learnable embeddings
		compute_locally.contiguous().data<bool>(),
		out_color.contiguous().data<float>(),
		out_embeddings.contiguous().data<float>(),//NOTE XYZ: Learnable embeddings
  ...
  return std::make_tuple(rendered, out_color, out_embeddings, n_render, n_consider, n_contrib, geomBuffer, binningBuffer, imgBuffer);
}
...
RenderGaussiansBackwardCUDA(
	...
	const torch::Tensor& dL_dout_embeddings, //NOTE XYZ: Learnable embeddings
	const torch::Tensor& means2D,// (P, 2)
	const torch::Tensor& conic_opacity,
	const torch::Tensor& rgb,
	const torch::Tensor& emb, //NOTE XYZ: Learnable embeddings
	const bool debug,
	const pybind11::dict &args)
{
  const int P = means2D.size(0);
  ...
  torch::Tensor dL_dembeddings = torch::zeros({P, EMB_CHANNELS}, means2D.options());//NOTE XYZ: Learnable embeddings 
  ...
  if(P != 0)
  {
	  CudaRasterizer::Rasterizer::renderBackward(
		...
		dL_dout_embeddings.contiguous().data<float>(),//NOTE XYZ: Learnable embeddings
		dL_dmeans2D.contiguous().data<float>(),
		dL_dconic.contiguous().data<float>(),
		dL_dopacity.contiguous().data<float>(),
		dL_dcolors.contiguous().data<float>(),//gradient of inputs
		dL_dembeddings.contiguous().data<float>(),//NOTE XYZ: Learnable embeddings
		reinterpret_cast<float2*>(means2D.contiguous().data<float>()),
		reinterpret_cast<float4*>(conic_opacity.contiguous().data<float>()),
		rgb.contiguous().data<float>(),
		emb.contiguous().data<float>(),//NOTE XYZ: Learnable embeddings
  ...
  return std::make_tuple(dL_dmeans2D, dL_dconic_opacity, dL_dcolors, dL_dembeddings);
}
