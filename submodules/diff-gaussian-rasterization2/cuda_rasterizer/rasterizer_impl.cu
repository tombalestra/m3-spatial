...
int CudaRasterizer::Rasterizer::renderForward(
	...
	float4* conic_opacity,
	float* rgb,
	float* emb, //NOTE XYZ: Learnable embeddings
	bool* compute_locally,
	float* out_color,
	...
	const pybind11::dict &args)
{
	...
	const float* embedding_ptr = emb; //NOTE XYZ: Learnable embeddings
	...
	CHECK_CUDA(FORWARD::render(//TODO: only deal with local tiles. do not even load other tiles.
		...
		means2D,
		feature_ptr,
		embedding_ptr, //NOTE XYZ: Learnable embeddings
		conic_opacity,
...
void CudaRasterizer::Rasterizer::renderBackward(
	...
	const float* dL_dembpix,//NOTE XYZ: Learnable embeddings
	...
	const float* emb,//NOTE XYZ: Learnable embeddings
	bool debug,
	const pybind11::dict &args)
{
	auto [global_rank, world_size, iteration, log_interval, device, zhx_debug, zhx_time, mode, dist_division_mode, log_folder] = prepareArgs(args);
	...
	CHECK_CUDA(BACKWARD::render(
		...
		compute_locally,
		dL_dpix,
		dL_dembpix, //NOTE XYZ: Learnable embeddings
		(float3*)dL_dmean2D,
		(float4*)dL_dconic,
		dL_dopacity,
		dL_dcolor,
