...
template <uint32_t CHANNELS, uint32_t ECHANNELS>
...
renderCUDA(
	...
	float* __restrict__ out_emb //NOTE XYZ: Learnable embeddings
	)
{
	...
	float C[CHANNELS] = { 0 };
	float E[ECHANNELS] = { 0 }; //NOTE XYZ: Learnable embeddings
	...
	for (int i = 0; i < rounds; i++, toDo -= BLOCK_SIZE)
	{
		...
		for (int j = 0; !done && j < min(BLOCK_SIZE, toDo); j++)
		{
			...
			for (int ch = 0; ch < ECHANNELS; ch++)
				E[ch] += embeddings[collected_id[j] * ECHANNELS + ch] * alpha * T;
...
void FORWARD::render(
	...
	float* out_color,
	float* out_emb //NOTE XYZ: Learnable embeddings
	)
{
	renderCUDA<NUM_CHANNELS, EMB_CHANNELS><<<grid, block >>>(
		...
		bg_color,
		out_color,
		out_emb //NOTE XYZ: Learnable embeddings
		);
}
