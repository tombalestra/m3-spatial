...
class ModelParams(ParamGroup):
    def __init__(self, parser, sentinel=False):
        ...
        self.use_clip = False
        self.clip_dim = 768
        self.clip_mem = "mem80.emb"
        self.clip_bit = [0, 16]
        
        self.use_siglip = False
        self.siglip_dim = 1152
        self.siglip_mem = "mem85.emb"
        self.siglip_bit = [16, 48]

        self.use_dinov2 = False
        self.dinov2_dim = 1536
        self.dinov2_mem = "mem75.emb"
        self.dinov2_bit = [48, 80]

        self.use_seem = False
        self.seem_dim = 512
        self.seem_mem = "mem100.emb"
        self.seem_bit = [80, 96]

        self.use_llama3 = False
        self.llama3_dim = 4096
        self.llama3_mem = "mem100.emb"
        self.llama3_bit = [96, 128]
        
        self.use_llamav = False
        self.llamav_dim = 4096
        self.llamav_mem = "mem60.emb"
        self.llamav_bit = [128, 160]
        
        self.emb_degree = 160 # NOTE XYZ: Learnable Feature
        self.softmax_temp = 0.05
        self.max_resolution = [980, 720]
...
def init_args(args):
    ...
    # add a date-time folder to avoid overwriting.
    # we need to figure out some useful parameters from args.
    # bsz, gpu_number, use_emb, use_clip, use_siglip, use_dinov2, use_seem, use_llama3, emb_degree, softmax_temp
    if not args.render:
        param_string = "{}_bsz{}_gpu{}_emb{}_clip{}_sig{}_dino{}_seem{}_lla{}_llv{}_dim{}_temp{}_".format(
            args.source_path.split('/')[-1], args.bsz, utils.WORLD_SIZE, args.use_embed, args.use_clip, args.use_siglip, args.use_dinov2, args.use_seem, args.use_llama3, args.use_llamav, args.emb_degree, args.softmax_temp
        )
        param_string += args.postfix
        model_path = os.path.join(args.model_path, param_string)
        os.makedirs(model_path, exist_ok=True)
        # os look up folder model_path, and find the largest run_id, then create a new folder with run_id+1, the initial id is 0, and zfill to 4.
        # Get all existing run folders
        existing_runs = [d for d in os.listdir(model_path) if os.path.isdir(os.path.join(model_path, d)) and d.startswith('run_')]
        new_id = max([int(run[4:]) for run in existing_runs], default=-1) + 1
        new_run_folder = f'run_{new_id:04d}'
        args.model_path = os.path.join(model_path, new_run_folder)
        ...
        args.visual_path = os.path.join(args.model_path, "visual")
    else:
