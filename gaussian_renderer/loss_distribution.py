...
from utils.loss_utils import pixelwise_l1_with_mask, pixelwise_ssim_with_mask, point_sample_cos_with_mask
import time
...
def load_camera_from_cpu_to_all_gpu(batched_cameras, batched_strategies, gpuid2tasks):
    ...
    def load_camera_from_cpu_to_gpu(first_task, last_task):
        coverage_min_max_y = {}
        # those are computing for how large the image should be loaded to GPU. Max Height: 560, Max Width: 960
        ...
        for camera_id_in_batch in range(first_task[0], last_task[0] + 1):
            ...
            # TODO: Temp Solution
            # Adapt to all model types
            batched_cameras[camera_id_in_batch].embeddings = {
                key: value.cuda() for key, value in batched_cameras[camera_id_in_batch].original_embeddings_backup.items()
            }
            coverage_min_max_y[camera_id_in_batch] = (coverage_min_y, coverage_max_y)
        return coverage_min_max_y

    if args.distributed_dataset_storage: # This is False
        if args.local_sampling:
            # TODO: may preloaded
    ...
    if args.local_sampling: # this is False
        return
...
def final_system_loss_computation(
    image, embeddings, viewpoint_cam, compute_locally, strategy, statistic_collector, emb_proj_ops, emb_mem_ops, args
):
    timers = utils.get_timers()
    args = utils.get_args()
    ...
    # Temp Solution
    # This is a current simple distillation loss, we need to fix it.
    timers.start("prepare_gt_embeddings")
    local_image_rect_gt = torch.clamp(viewpoint_cam.original_image / 255.0, 0.0, 1.0)
    timers.stop("prepare_gt_embeddings")
    
    timers.start("prepare_loss_embeddings")
    if embeddings is None:
        L_embs = {}
    else:
        local_embeddings_rect = embeddings[:, coverage_min_y:coverage_max_y, :].contiguous() # NOTE XYZ: Learnable embeddings
        L_embs = point_sample_cos_with_mask(local_embeddings_rect, viewpoint_cam.embeddings, emb_proj_ops, emb_mem_ops, args)
    timers.stop("prepare_loss_embeddings")
    return Ll1, ssim_loss, L_embs


def batched_loss_computation(
    batched_image,
    batched_embeddings, # NOTE XYZ: Learnable embeddings
    batched_cameras,
    ...
    for idx, (
        image,
        embeddings, # NOTE XYZ: Learnable embeddings
        camera,
        compute_locally,
        strategy,
        statistic_collector,
        ...
        if image is None:  # This image is not rendered locally.
            loss = 0
            losses = {"Ll1": loss, "ssim_loss": 0.0}
            models = ['clip', 'llama3', 'siglip', 'dinov2', 'seem', "llamav"]
            for model in models:
                if getattr(args, f'use_{model}'):
                    losses.update({
                        f"{model}_cosine_loss": 0.0,
                        f"{model}_l2_loss": 0.0,
                    })
            ...
            Ll1, ssim_loss, L_embs = final_system_loss_computation(
                image, embeddings, camera, compute_locally, strategy, statistic_collector, emb_proj_ops, emb_mem_ops, args
            ) # NOTE XYZ: Learnable embeddings
            loss = (1.0 - args.lambda_dssim) * Ll1 \
                    + args.lambda_dssim * (1.0 - ssim_loss)
            for key, value in L_embs.items():
                loss += (value * args.lambda_emb)
            # NOTE XYZ: Learnable embeddings
            # print(f"ssim_loss: {1-ssim_loss}")
            losses = {"Ll1": Ll1, "ssim_loss": ssim_loss}
            losses.update(L_embs)
            batched_losses.append(losses)
        loss_sum += loss

    dict_losses = {}
    for batched_loss in batched_losses:
        for key, value in batched_loss.items():
            if key not in dict_losses:
                dict_losses[key] = []
            dict_losses[key].append(value.item() if isinstance(value, torch.Tensor) else value)
    for key, value in dict_losses.items():
        pos_count = sum(torch.tensor(value) > 0).item()
        if pos_count == 0:
            dict_losses[key] = 0
        else:
            dict_losses[key] = sum(value) / sum(torch.tensor(value) > 0).item()

    assert loss_sum.dim() == 0, "The loss_sum must be a scalar tensor."
    timers.stop("loss_computation")
    return loss_sum * args.lr_scale_loss, batched_losses, dict_losses
