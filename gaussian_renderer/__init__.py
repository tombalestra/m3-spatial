...
from diff_gaussian_rasterization2 import GaussianRasterizationSettings as GaussianRasterizationSettings2
from diff_gaussian_rasterization2 import GaussianRasterizer as GaussianRasterizer2
...
def replicated_preprocess3dgs(
    ...
    shs = pc.get_features
    emb = pc.get_embeddings # NOTE XYZ: Learnable Feauture
    if timers is not None:
    ...
    screenspace_pkg = {
        ...
        "rgb_for_render": rgb,
        "emb_for_render": emb, # NOTE XYZ: Learnable Feauture
...
def render(screenspace_pkg, strategy=None):
    ...
    if screenspace_pkg["means2D_for_render"].shape[0] < 1000:
        ...
        # TODO XYZ: Add learnable feature for gaussians render.
    if timers is not None:
...
def all_to_all_communication_final(
    ...
    for k in range(num_cameras):
        ...
        means2D, rgb, emb, conic_opacity, radii, depths = batched_screenspace_params[k] # NOTE XYZ: Learnable Feature
    ...
    for k in range(num_cameras):
        ...
        if k == 0:
            mean2d_dim1 = means2D.shape[1]
            rgb_dim1 = rgb.shape[1]
            emb_dim1 = emb.shape[1] # NOTE XYZ: Learnable Feauture
            conic_opacity_dim1 = conic_opacity.shape[1]
        batched_catted_screenspace_states.append(
            torch.cat([means2D, rgb, emb, conic_opacity], dim=1).contiguous()
        ) # NOTE XYZ: Learnable Feauture
        batched_catted_screenspace_auxiliary_states.append(
            torch.cat(
                [radii.float().unsqueeze(1), depths.unsqueeze(1)], dim=1
    ...
    batched_emb_redistributed = [] # NOTE XYZ: Learnable Feauture
    batched_conic_opacity_redistributed = []
    for k in range(num_cameras):
        means2D_redistributed, rgb_redistributed, emb_redistributed, conic_opacity_redistributed = (
            torch.split(
                batched_params_redistributed[k],
                [mean2d_dim1, rgb_dim1, emb_dim1, conic_opacity_dim1],
                dim=1,
            )
        ) # NOTE XYZ: Learnable Feauture
        batched_means2D_redistributed.append(means2D_redistributed)
        batched_rgb_redistributed.append(rgb_redistributed)
        batched_emb_redistributed.append(emb_redistributed) # NOTE XYZ: Learnable Feauture
...
def distributed_preprocess3dgs_and_all2all_final(
    ...
    scales = pc.get_scaling
    rotations = pc.get_rotation
    use_embed = pc.use_embed
    shs = pc.get_features
    emb = pc.get_embeddings[:,0] # TODO XYZ: Learnable Feature, to see any future problem for hard code [:,0]
    if timers is not None:
        timers.stop("forward_prepare_gaussians")
    utils.check_initial_gpu_memory_usage("after forward_prepare_gaussians")
    ########## [END] Prepare Gaussians for rendering ##########
    ...
    for i, (viewpoint_camera, strategy) in enumerate(
        ...
        kwargs = {
            ...
            "projmatrix": viewpoint_camera.full_proj_transform,
            "sh_degree": pc.active_sh_degree,
            "emb_degree": pc.emb_degree,  # NOTE XYZ: Learnable Feature
            "campos": viewpoint_camera.camera_center,
            "prefiltered": False,
            "debug": pipe.debug,
        }
        ...
        means2D, rgb, conic_opacity, radii, depths = rasterizer.preprocess_gaussians(
            ...
            opacities=opacity,
            cuda_args=cuda_args,
        ) # NOTE XYZ: Learnable Feature
        if mode == "train":
            means2D.retain_grad()
        batched_means2D.append(means2D)
        screenspace_params = [means2D, rgb, emb, conic_opacity, radii, depths] # NOTE XYZ: Learnable Feature
        batched_rasterizers.append(rasterizer)
        batched_screenspace_params.append(screenspace_params)
        batched_radii.append(radii)
    utils.check_initial_gpu_memory_usage("after forward_preprocess_gaussians")
    ...
    batched_screenspace_pkg = {
        ...
        "batched_emb_redistributed": batched_emb_redistributed, # NOTE XYZ: Learnable Feature
...
def render_final(batched_screenspace_pkg, batched_strategies, use_embed=False, tile_size=16):
    """
    Render the scene.
    """
    timers = utils.get_timers()
    ...
    for cam_id in range(len(batched_screenspace_pkg["batched_rasterizers"])):
        ...
        if utils.GLOBAL_RANK not in strategy.gpu_ids:
            ...
            batched_rendered_embeddings.append(None)
        ...
        emb_redistributed = batched_screenspace_pkg["batched_emb_redistributed"][cam_id] # NOTE XYZ: Learnable Feature
        ...
        if means2D_redistributed.shape[0] < 10:
            ...
            kwargs = {
                ...
                "emb": emb_redistributed,  # NOTE XYZ: Learnable Feature
            ...
            results = (
                rasterizer.render_gaussians(**kwargs)
            )
            
            if use_embed:
                rendered_image, rendered_embeddings, n_render, n_consider, n_contrib = results
            else:
                rendered_image, n_render, n_consider, n_contrib = results
                rendered_embeddings = None

        batched_rendered_image.append(rendered_image)
        batched_rendered_embeddings.append(rendered_embeddings)
    ...
    ########## [END] CUDA Rasterization Call ##########
    return batched_rendered_image, batched_rendered_embeddings, batched_compute_locally
