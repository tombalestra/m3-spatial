...
embedding_functions = {
    'use_clip': load_clip,
    'use_dinov2': load_dinov2,
    'use_llama3': load_llama3,
    'use_seem': load_seem,
    'use_siglip': load_siglip,
    'use_llamav': load_llamav,
}
...
def loadCam(args, id, cam_info, decompressed_image=None, return_image=False):
    ...
    # NOTE: we do resolution rescaling here.
    ...
    if (
        ...
        # TODO: This fuction is quite strange.
        resized_image_rgb, embeddings = PILtoTorch(
            image, resolution, args, log_file, decompressed_image=decompressed_image
        )
        
        if embeddings is None and (not (hasattr(args, 'apply_trace') and args.apply_trace)):
            embeddings = {}
            for arg_name, load_function in embedding_functions.items():
                if getattr(args, arg_name):
                    # since thye do not have upsample, downsample of the image
                    embeddings.update(load_function(args, cam_info.embed_info, cam_info.image_path))
    ...
    if return_image:
        return {"gt_image": gt_image, "embeddings": embeddings}
...
# NOTE XYZ: This is used for single process loading debug 
def decompressed_images_from_camInfos_multiprocess(cam_infos, args):
    args = get_args()  # Assuming get_args() is defined elsewhere
    decompressed_images = []
    total_cameras = len(cam_infos)
    ...
    # Process tasks in a single process
    results = list(
        tqdm(
            (load_decompressed_image(task) for task in tasks),
            total=total_cameras,
            disable=(utils.LOCAL_RANK != 0),
        )
    )

    # You can set a breakpoint here to inspect results
