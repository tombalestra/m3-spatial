...
def PILtoTorch(pil_image, resolution, args, log_file, decompressed_image=None):
    if decompressed_image is not None:
        return decompressed_image['gt_image'], decompressed_image['embeddings']
    pil_image.load()
    ...
    if len(resized_image.shape) == 3:
        return resized_image.permute(2, 0, 1), None
    else:
        return resized_image.unsqueeze(dim=-1).permute(2, 0, 1), None
