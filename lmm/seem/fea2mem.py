...
def load_seem_embeddings(data_root, device):
    json_path = os.path.join(data_root, "seem_info.json")
    with open(json_path, "r") as f:
        info = json.load(f)
    
    all_embeddings = []
    for image_info in tqdm(info["images"], desc="Loading SEEM embeddings"):
...
if __name__ == "__main__":
    ...
    print("Loading SEEM embeddings...")
    all_embeddings = load_seem_embeddings(data_root, device)
    ...
    output_path = os.path.join(data_root, "seem", f"mem{int(threshold*100)}.emb")
