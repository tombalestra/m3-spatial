...
def load_dinov2_embeddings(data_root, device):
    ...
    all_embeddings = []
    all_image_pths = []
    for image_info in tqdm(info["images"], desc="Loading DINOv2 embeddings"):
...
if __name__ == "__main__":
    ...
    all_embeddings, all_image_pths = load_dinov2_embeddings(data_root, device)
    
    # 0.7, 0.75, 0.80, 0.85, 0.90
    threshold = 0.9
    mem_path = os.path.join(data_root, "dinov2", f"mem{int(threshold*100)}.emb")
    ...
    mem2fea_path = os.path.join(data_root, "dinov2", f"mem{int(threshold*100)}.index")
