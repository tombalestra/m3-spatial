...
class CLIPVisionTransformer(nn.Module):
    ...
    def forward(
        ...
        pooled_feature = last_hidden_state[:, 1:, :]
        n,d,l = pooled_feature.shape
        pooled_feature = self.post_layernorm(pooled_feature.view(n*d,l))
        pooled_feature = pooled_feature.view(n,d,l)

        return BaseModelOutputWithPooling(
            last_hidden_state=last_hidden_state,
            pooler_output=(pooled_output, pooled_feature),
            hidden_states=encoder_outputs.hidden_states,
            attentions=encoder_outputs.attentions,
        )
...
class CLIPModel(CLIPPreTrainedModel):
    ...
    def forward(
        ...
        image_embeds = vision_outputs[1][0]
        image_embeds = self.visual_projection(image_embeds)
        
        mask_embeds = vision_outputs[1][1]
        n,d,c = mask_embeds.shape
        mask_embeds = self.visual_projection(mask_embeds.view(n*d,c)).view(n,d,-1)
        ...
        return CLIPOutput(
            ...
            image_embeds=(image_embeds, mask_embeds),
