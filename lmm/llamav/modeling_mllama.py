...
class MllamaVisionModel(MllamaPreTrainedModel):
    ...
    def forward(
        ...
        hidden_state = self.apply_class_embedding(hidden_state) # The first token is the class embedding
        num_patches += 1
        ...
        num_padding_patches = (8 - (hidden_state.shape[-2] % 8)) % 8 # because of class embedding, we usually need to pad 7 here.
        ...
        # output states contain 1280 dim for hidden states and 6400 dim for intermediate hidden states
        return BaseModelOutput(
...
class MllamaForConditionalGeneration(MllamaPreTrainedModel, GenerationMixin):
    ...
    def forward(
        ...
        outputs = self.language_model(
            ...
            attention_mask=attention_mask, # Vision
            position_ids=position_ids, # Vision
            cross_attention_states=cross_attention_states, # Projected, Vision
            cross_attention_mask=cross_attention_mask, # Vision, Language
            full_text_row_masked_out_mask=full_text_row_masked_out_mask, # Vision
            past_key_values=past_key_values, # Lang
            use_cache=use_cache,
            inputs_embeds=inputs_embeds,
            labels=labels,
            output_hidden_states=output_hidden_states,
