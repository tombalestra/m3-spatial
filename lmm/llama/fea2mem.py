...
def load_llama3_embeddings(data_root, device):
    json_path = os.path.join(data_root, "llama3_info.json")
    with open(json_path, "r") as f:
        info = json.load(f)
    
    all_embeddings = []
    for image_info in tqdm(info["images"], desc="Loading LLaMA embeddings"):
        emb_path = os.path.join(data_root, "/".join(image_info["emb_pth"].split('/')[-3:]))
        embeddings = torch.load(emb_path)
        acc_embeddings = []
        for value in embeddings.values():
            acc_embeddings += [value.to(device)]
        if len(acc_embeddings) == 0:
            continue
        pixel_embeds = torch.stack(acc_embeddings)
...
if __name__ == "__main__":
    data_root = "/home/xuanbin/ICLR25/data/drjohnson"  # Update this path as needed
    ...
    all_embeddings = load_llama3_embeddings(data_root, device)
    ...
    output_path = os.path.join(data_root, "llama3", f"mem{int(threshold*100)}.emb")
